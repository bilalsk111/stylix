...
export const QuickViewModal = ({ product, isOpen, onClose, onQuickAdd, wishlisted, onWishlist, addedToCart }) => {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 sm:p-6 bg-stone-900/60 backdrop-blur-sm animate-in fade-in duration-200">
      ...
      <div 
        className="relative w-full max-w-[900px] max-h-[90vh] md:h-[550px] flex flex-col md:flex-row bg-white shadow-2xl animate-in zoom-in-95 duration-200 overflow-hidden border border-stone-200"
        onClick={(e) => e.stopPropagation()}
      >
        <button 
          onClick={onClose} 
          className="absolute top-4 right-4 z-50 p-2 text-stone-500 bg-white/90 backdrop-blur-md border border-stone-200 shadow-sm hover:text-stone-900 hover:bg-white rounded-full transition-all"
        >
        ...
        <div className="w-full md:w-1/2 h-full p-6 md:p-8 flex flex-col text-stone-900 overflow-y-auto custom-scrollbar">
          ...
          <div className="mb-5 pr-8">
            {/* 🔥 BRAND ADDED HERE */}
            <span className="text-[10px] text-stone-500 font-black tracking-[0.2em] uppercase mb-1.5 block">
              {product.brand || "Stylix"}
            </span>
            <h2 className="text-lg md:text-xl font-black uppercase tracking-widest mb-2 leading-snug text-stone-900 line-clamp-3">
              {product.title}
            </h2>
              ...
              <div className="flex flex-wrap gap-2">
                {sizes.map(size => (
                  <button
                    ...
                    className={`min-w-[40px] h-9 px-3 flex items-center justify-center text-[10px] font-black tracking-widest uppercase transition-all border ${
                      selectedSize === size 
                        ? "border-stone-900 bg-stone-900 text-white shadow-md" 
                        : "border-stone-200 bg-white text-stone-500 hover:border-stone-900 hover:text-stone-900"
                    }`}
          ...
          {/* 🔥 GAP FIXED: mt-auto hata kar natural mt-6 lagaya hai */}
          <div className="mt-6 pt-5 border-t border-stone-100">
            <div className="flex gap-3 mb-4">
              <button 
                onClick={handleCartAction}
                disabled={product.stock === 0}
                className={`flex-1 flex items-center justify-center gap-2 py-3.5 text-[11px] font-black uppercase tracking-widest transition-all disabled:opacity-50 disabled:cursor-not-allowed ${
                  isAdded 
              ...
              <button 
                ...
                className={`w-14 flex items-center justify-center border transition-all ${
                  wishlisted 
                    ...
                    : "border-stone-200 text-stone-400 hover:border-stone-900 hover:text-stone-900 hover:bg-stone-50"
                }`}
              >
            ...
            <button 
              ...
              className="text-[10px] font-black text-stone-400 uppercase tracking-widest hover:text-stone-900 transition-colors flex items-center gap-2 group"
            >
              VIEW FULL DETAILS 
