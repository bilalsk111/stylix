...
/* ─────────────────────────────────────────────────────────────────
   VARIANT CARD  — onDelete now receives (index, variant)
───────────────────────────────────────────────────────────────── */
...
const SellerProductDetails = () => {
  ...
  const [deleteTarget, setDeleteTarget] = useState(null); // { index, variant }
            ...
            <div className="flex flex-col gap-2 w-16 no-scrollbar overflow-y-auto">
              {product?.images?.map((img, i) => (
                <button key={i} onClick={() => setActiveImg(i)}
                  className={`aspect-[3/4] w-full rounded-md overflow-hidden border transition-all ${activeImg === i ? "border-stone-900 ring-1 ring-stone-900" : "border-stone-200 opacity-60 hover:opacity-100"}`}>
                  <img src={img.url} className="w-full h-full object-cover" alt="" />
                </button>
              ))}
            </div>
            <div className="flex-1 bg-stone-100 rounded-2xl overflow-hidden relative border border-stone-200 shadow-sm">
            ...
            <section className="bg-stone-50 border border-stone-200 rounded-2xl p-6 shadow-sm">
              <div className="flex items-center justify-between mb-6">
                ...
                <button onClick={() => toggleModal()}
                  className="bg-stone-900 text-white px-5 py-2.5 text-[8px] font-black uppercase tracking-widest rounded-md hover:bg-[#ccff00] hover:text-stone-900 transition-colors shadow-sm">
                  + Add Node
                </button>
              </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-stone-900/40 backdrop-blur-sm">
          <div className="bg-white border border-stone-200 w-full max-w-md rounded-3xl overflow-hidden flex flex-col max-h-[90vh] shadow-2xl">
            <div className="p-6 border-b border-stone-100 flex justify-between items-center shrink-0 bg-stone-50">
              ...
              <button onClick={() => { setIsModalOpen(false); setEditingIndex(null); }} className="text-stone-400 hover:text-stone-900 transition-colors">
                <X size={18} />
              </button>
            </div>
            ...
            <div className="p-8 space-y-6 overflow-y-auto no-scrollbar">
              <div className="space-y-3 p-1">
                <div className="flex items-baseline justify-between">
                  <label className="text-[8px] font-black uppercase text-stone-500 tracking-widest">Variant Imagery (Max 7)</label>
                ...
                <div className="flex gap-3 overflow-x-auto no-scrollbar py-1">
                  {variantForm.images?.map((img, idx) => (
                    <div key={idx} className="relative w-16 h-20 shrink-0 border border-stone-200 rounded-lg overflow-hidden group">
                      <img src={img.url} className="w-full h-full object-cover" alt="" />
                      <button onClick={() => removeImage(idx)}
                        className="absolute top-1.5 right-1.5 bg-white/90 p-1 rounded-sm opacity-0 group-hover:opacity-100 transition-opacity border border-stone-200 shadow-sm">
                        <X size={10} className="text-stone-900" />
                      </button>
                    </div>
                  ))}
                  {(variantForm.images?.length || 0) < 7 && (
                    <label className="w-16 h-20 shrink-0 border border-dashed border-stone-300 rounded-lg flex flex-col gap-1 items-center justify-center cursor-pointer group hover:border-stone-900 transition-colors bg-stone-50">
                      <ImagePlus size={16} className="text-stone-400 group-hover:text-stone-900" />
                      <span className="text-[6px] font-black uppercase tracking-[0.3em] text-stone-400 group-hover:text-stone-900">ADD</span>
              ...
              <div className="space-y-1 p-1">
                ...
                <input type="text" value={variantForm.title} onChange={e => setVariantForm(p => ({ ...p, title: e.target.value }))}
                  className="w-full glass-input p-3 rounded-lg text-[10px] font-bold uppercase placeholder:text-stone-400" placeholder="Variant SKU Title" />
              </div>
                ...
                {["SIZE", "COLOR"].map(key => (
                  <div key={key} className="space-y-1">
                    ...
                    <input type="text" value={variantForm.attributes[key]}
                      ...
                      className="w-full glass-input p-3 rounded-lg text-[10px] font-bold placeholder:text-stone-400" placeholder={`Cluster ${key}`} />
                  </div>
                ))}
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 p-1">
                <div className="space-y-1">
                  <label className="text-[8px] font-black uppercase text-stone-500 tracking-widest">Stock</label>
                  <input type="number" value={variantForm.stock} onChange={e => setVariantForm(p => ({ ...p, stock: e.target.value }))}
                    className="w-full glass-input p-3 rounded-lg text-[10px] font-mono placeholder:text-stone-400" placeholder="UNITS_IN_VAULT" />
                </div>
                <div className="space-y-1">
                  <label className="text-[8px] font-black uppercase text-stone-500 tracking-widest">Currency</label>
                  <select value={variantForm.price.currency} onChange={e => setVariantForm(p => ({ ...p, price: { ...p.price, currency: e.target.value } }))}
                    className="w-full glass-input p-3 rounded-lg text-[10px] font-bold appearance-none cursor-pointer">
              ...
              <div className="space-y-1 p-1">
                <label className="text-[8px] font-black uppercase text-stone-500 tracking-widest">Price</label>
                <input type="number" value={variantForm.price.amount}
                  onChange={e => setVariantForm(p => ({ ...p, price: { ...p.price, amount: e.target.value } }))}
                  className="w-full bg-stone-100 border border-stone-200 focus:border-stone-900 focus:bg-white p-4 text-2xl font-black text-stone-900 italic outline-none rounded-xl transition-all placeholder:text-stone-300"
                  placeholder="0.00" />
              </div>
            </div>
